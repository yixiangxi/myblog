<template>
  <div>
    ...
    <!--    搜索-->
    <div class="search">
      <el-input placeholder="请输入博客标题查询" style="width: 200px;margin-right: 5px" v-model="title"></el-input>
      <el-input placeholder="请输入博主名称查询" style="width: 200px;margin-right: 5px" v-model="userName"></el-input>
      <el-input placeholder="请输入博客分类查询" style="width: 200px" v-model="categoryName"></el-input>
    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="tags" label="标签">
          <template v-slot="scope">
            <el-tag v-for="item in JSON.parse(scope.row.tags||'[]')" :key="item" style="margin: 2px">{{ item }}</el-tag>
          </template>
        </el-table-column>
    ...
    <!--新增数据表单-->
    <el-dialog title="博客分类" :visible.sync="fromVisible" width="50%" :close-on-click-modal="false" destroy-on-close>
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        ...
        if (valid) {
          this.form.tags = JSON.stringify(this.tagsArr)//把json数组转化为字符串存储到数据库
          this.form.content = this.editor.txt.html()//将富文本编辑框中的内容存储到表中的content中返回到后端存储到数据库中
          this.$request({
